...
- Notification (When the server starts, it creates a message queue for users using a ```node schedule```. The backend then processes the queue and sends emails to the respective users. After successful sending, the server updates the message status in the database.)