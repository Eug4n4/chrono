...
const CalendarView = React.forwardRef(
    ({ onDatesSet, filterTypes, filterTags }, ref) => {
        ...
        const fetchEventsForYears = (years) => {
            ...
            years.forEach((year) => {
                const calendarsToFetch = validCalendarIds.filter(
                    (calId) =>
                        !fetchedEventKeys.current.has(`${year}-${calId}`),
                );
        ...
        const combinedEvents = activeCalendars
            ...
            .filter((event) => {
                ...
                if (filterTags && filterTags.length > 0) {
                    ...
                    const eventTagIds = eventTags.map((t) => t._id);
                    const hasMatchingTag = filterTags.some((tagId) =>
