...
const EventCreateFormModal = ({
    ...
    const [date, setDate] = useState({
        start: {
            date: defaultDate.start.date || "",
            time: defaultDate.start.time || "",
        },
        end: { date: "", time: "" },
        reminder: { date: "", time: "" },
    });
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (!date.start.date || !date.start.time)
                return showWarningToast("Input start date!");
            ...
            if (type === "task") {
                ...
                if (!date.end.date || !date.end.time)
                    return showWarningToast("Input end date!");
                ...
                if (startDate <= reminderDate)
                    return showWarningToast(
                        "Reminder cannot be later than start date!",
                    );
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            ...
            <div className={styles.buttonGroup}>
                {step > 1 && (
                    <button className={styles.send_create} onClick={prevStep}>
                        Back
                    </button>
                )}
                {step < 3 && (
                    <button className={styles.send_create} onClick={nextStep}>
                        Next
                    </button>
                )}
                {step === 3 && (
